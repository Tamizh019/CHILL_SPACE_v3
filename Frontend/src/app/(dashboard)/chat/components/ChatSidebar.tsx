            ...
            {isCollapsed ? (
                ...
                <div className="flex flex-col items-center py-6 gap-6 h-full">
                    <button
                        ...
                        className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center text-slate-400 hover:text-white transition-all shadow-lg"
                    ...
                    <div className="flex flex-col gap-3 w-full items-center">
                        <button
                            onClick={() => { onTabChange('spaces'); setIsCollapsed(false); }}
                            className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${activeTab === 'spaces' ? 'bg-violet-600 shadow-violet text-white' : 'hover:bg-white/5 text-slate-500 hover:text-white'}`}
                            title="Spaces"
                        >
                            <span className="material-icons-round text-xl">diversity_3</span>
                        </button>
                        <button
                            onClick={() => { onTabChange('files'); setIsCollapsed(false); }}
                            className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${activeTab === 'files' ? 'bg-violet-600 shadow-violet text-white' : 'hover:bg-white/5 text-slate-500 hover:text-white'}`}
                            title="Files"
                        >
                            <span className="material-icons-round text-xl">folder</span>
                        </button>
                        <button
                            onClick={() => { onTabChange('dms'); setIsCollapsed(false); }}
                            className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${activeTab === 'dms' ? 'bg-violet-600 shadow-violet text-white' : 'hover:bg-white/5 text-slate-500 hover:text-white'}`}
                ...
                <>
                    ...
                    <div className="p-6 pb-4 flex-shrink-0">
                        <div className="flex items-center justify-between mb-6">
                            <h1 className="font-heading text-2xl font-bold text-white">Chat & Files</h1>
                            <button
                                onClick={() => setIsCollapsed(true)}
                                className="w-8 h-8 rounded-lg hover:bg-white/5 flex items-center justify-center text-slate-500 hover:text-white transition-colors"
                    ...
                    {/* Community Rules */}
                    {activeTab !== 'files' && (
                        <div className="border-t border-white/5 bg-black/10 flex-shrink-0">
                            <button
