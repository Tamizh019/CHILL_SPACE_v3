...
export function GlobalStoreProvider({ children }: { children: ReactNode }) {
    ...
    const fetchUser = useCallback(async (force = false) => {
        ...
        return null;
    }, [store.userProfile.data, store.userProfile.lastFetched]);
    ...
    }, [store.friends.data.length, store.friends.lastFetched, store.userProfile.data]);
    ...
    }, [store.channels.data.length, store.channels.lastFetched]);
    ...
    useEffect(() => {
        ...
        const init = async () => {
            // In RN, we might start loading before auth check, but auth check is fast
            // But if we are not authenticated, getUser returns null.
            try {
                ...
                console.log("Error init store", e)
            } finally {
