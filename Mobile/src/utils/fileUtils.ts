// File utility functions for Chill Space v3 Mobile
...
export function getFileTypeInfo(mimeType: string): FileTypeInfo {
    ...
    if (mimeType.startsWith('video/')) {
        return { icon: 'video', color: 'text-purple-400', category: 'video' };
    }
    if (mimeType.startsWith('audio/')) {
        return { icon: 'music', color: 'text-pink-400', category: 'audio' };
    }
    if (mimeType === 'application/pdf') {
        return { icon: 'file-text', color: 'text-red-500', category: 'document' };
    }
    if (mimeType === 'text/csv' || mimeType.includes('csv')) {
        return { icon: 'grid', color: 'text-green-500', category: 'document' };
    }
    if (mimeType.includes('word') || mimeType.includes('document')) {
        return { icon: 'file-text', color: 'text-blue-500', category: 'document' };
    }
    if (mimeType.includes('sheet') || mimeType.includes('excel')) {
        return { icon: 'grid', color: 'text-emerald-500', category: 'document' };
    }
    if (mimeType.includes('presentation') || mimeType.includes('powerpoint')) {
        return { icon: 'monitor', color: 'text-orange-500', category: 'document' };
    }
    if (mimeType.includes('zip') || mimeType.includes('rar') || mimeType.includes('tar') || mimeType.includes('compressed')) {
        return { icon: 'folder', color: 'text-amber-400', category: 'archive' };
    }
    ...
    return { icon: 'file', color: 'text-slate-400', category: 'other' };
}
...
// Replaced File with generic interface for RN
export function validateFileSize(file: { size: number }): { valid: boolean; error?: string } {
